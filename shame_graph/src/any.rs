...
impl Any {
    ...
    pub fn discard_fragment() {
        Context::with(|ctx| {
            let block = &mut ctx.blocks_mut()[ctx.current_block_key_unwrap()];
            if ctx.shader_kind == ShaderKind::Fragment {
                block.record_stmt(Stmt::new(RecordTime::next(), StmtKind::Discard))
            }
        });
    }
}
